...
import { CalibrateRequest, SessionState } from '@/lib/types';
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!session_id || !calibration_feedback) {
      return new Response(
        JSON.stringify({ error: 'Invalid request: session_id and calibration_feedback are required' }),
        { status: 400, headers: { 'Content-Type': 'application/json' } }
      );
    }
    
    console.log('[API /calibrate] Calibrating position for session:', session_id);
    ...
    const stream = new ReadableStream({
      async start(controller) {
        try {
          ...
          // Run AI strategy cycle (first cycle - calibration)
          const aiResponse = await runStrategyCycle(sessionState, calibration_feedback, true);
          ...
          console.log('[API /calibrate] Position calibrated successfully, new position:', new_arrow_position);
          ...
          console.error('[API /calibrate] Stream error:', error);
    ...
    if (error instanceof Error && error.message.includes('AI generation failed')) {
      return new Response(
        JSON.stringify({ error: 'Failed to calibrate position', details: error.message }),
